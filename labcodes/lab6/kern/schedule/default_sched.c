...
stride_pick_next(struct run_queue *rq) {
    ...
    if (le == &rq -> run_list)                
        return NULL;    
    struct proc_struct *target_proc = le2proc(le, run_link);
    struct proc_struct *tmp = NULL;
    le = list_next(le);
    ...
    // update
    target_proc -> lab6_stride += BIG_STRIDE / ((target_proc -> lab6_priority == 0) ? 1 : target_proc -> lab6_priority);
    return target_proc;
}
